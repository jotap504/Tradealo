...
const mockUsersService = {
  ...
  updateProfile: jest.fn().mockResolvedValue({ ...MOCK_PROFILE, bio: 'updated' }),
  getPublicProfile: jest.fn().mockResolvedValue({ id: 'user-1' }),
  getAvatarUploadUrl: jest.fn().mockResolvedValue({ uploadUrl: 'https://r2.example.com', key: 'k', expiresIn: 300 }),
  confirmAvatarUpload: jest.fn().mockResolvedValue({ avatarUrl: 'https://cdn.example.com/k' }),
  getKycUploadUrl: jest.fn().mockResolvedValue({ uploadUrl: 'https://r2.example.com', key: 'k', expiresIn: 300 }),
}
...
describe('UsersController', () => {
  ...
  beforeEach(async () => {
    ...
    Object.assign(mockUsersService, {
      ...
      updateProfile: jest.fn().mockResolvedValue({ ...MOCK_PROFILE, bio: 'updated' }),
      getPublicProfile: jest.fn().mockResolvedValue({ id: 'user-1' }),
      getAvatarUploadUrl: jest.fn().mockResolvedValue({ url: 'https://r2.example.com', fields: {}, key: 'k', expiresIn: 300 }),
      confirmAvatarUpload: jest.fn().mockResolvedValue({ avatarUrl: 'https://cdn.example.com/k' }),
      getKycUploadUrl: jest.fn().mockResolvedValue({ url: 'https://r2.example.com', fields: {}, key: 'k', expiresIn: 300 }),
    })
  ...
  it('updateProfile() delegates with userId and dto', async () => {
    ...
    expect(mockUsersService.updateProfile).toHaveBeenCalledWith('user-1', { bio: 'hola' })
  })
  ...
  it('confirmAvatarUpload() delegates with userId and key', async () => {
    await controller.confirmAvatarUpload(JWT_USER, { key: 'avatars/user-1/file.jpg' })
    expect(mockUsersService.confirmAvatarUpload).toHaveBeenCalledWith('user-1', 'avatars/user-1/file.jpg')
  })

  it('getKycUploadUrl() delegates with userId and type', async () => {
    await controller.getKycUploadUrl(JWT_USER, 'dni')
    expect(mockUsersService.getKycUploadUrl).toHaveBeenCalledWith('user-1', 'dni')
  })
})
