...
const TOKEN_PAIR = {
  ...
  expiresIn: 900,
  user: { id: 'user-1', email: 'test@example.com', role: 'user', kycLevel: 0, referralCode: 'CODE', createdAt: new Date() },
}

const mockAuthService = {
  register: jest.fn().mockResolvedValue(TOKEN_PAIR),
  login: jest.fn().mockResolvedValue(TOKEN_PAIR),
  refresh: jest.fn().mockResolvedValue({ accessToken: 'new_access', refreshToken: 'new_refresh', expiresIn: 900 }),
...
describe('AuthController', () => {
  ...
  it('login() delegates to AuthService.login', async () => {
    ...
    expect(mockAuthService.login).toHaveBeenCalledWith({ email: 'test@example.com', password: 'Password1' })
    expect(result).toEqual(TOKEN_PAIR)
  })
