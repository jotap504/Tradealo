...
async function runSeeds(): Promise<void> {
  ...
  for (const cat of roots) {
    const [inserted] = await db
      ...
      .values({
        name: cat.name,
        slug: cat.slug,
        isCollectible: cat.isCollectible,
      })
      .onConflictDoUpdate({
        target: schema.categories.slug,
        // Force parentId=null so categories that used to be children
        // (e.g. calzado) get promoted to root on re-seed.
        set: { name: cat.name, parentId: null },
      })
  ...
  console.log(
    `  ✓ ${CATEGORY_ATTRIBUTES_SEED.length} category attributes seeded`,
  );
