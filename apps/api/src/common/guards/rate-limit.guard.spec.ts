...
describe('RateLimitGuard', () => {
  ...
  it('uses user ID as key when keyBy=user and user is authenticated', async () => {
    mockReflector.getAllAndOverride.mockReturnValue({ ttl: 3600, limit: 20, keyBy: 'user' })
    await guard.canActivate(makeContext('1.2.3.4', { sub: 'user-001' }))
    expect(mockRedis.incr).toHaveBeenCalledWith(expect.stringContaining('u:user-001'))
  })
  ...
  it('falls back to IP when keyBy=user but user is not authenticated', async () => {
    ...
    expect(mockRedis.incr).toHaveBeenCalledWith(expect.stringContaining('ip:1.2.3.4'))
  })
