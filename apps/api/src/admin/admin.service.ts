...
import { eq, desc, and, gte, lt, ilike, asc, sql, or } from 'drizzle-orm'
...
export class AdminService {
  ...
  async getDashboardStats() {
    ...
    return {
      users: { total: totalUsers.length, lastThirtyDays: recentUsers.length },
      listings: { active: activeListings.length, pendingModeration: pendingListings.length },
  ...
  async listUsers(
    ...
    if (params.role) {
      conditions.push(eq(schema.users.role, params.role as typeof schema.users.$inferSelect['role']))
    }
    if (params.kycLevel !== undefined) {
      conditions.push(eq(schema.users.kycLevel, params.kycLevel))
    }
    if (params.status) {
      conditions.push(eq(schema.users.status, params.status as typeof schema.users.$inferSelect['status']))
    }
    if (params.search) {
  ...
  async getUser(userId: string) {
    ...
    const [profile, wallet, verifications] = await Promise.all([
      this.db.select().from(schema.userProfiles).where(eq(schema.userProfiles.userId, userId)).limit(1),
      this.db.select().from(schema.wallets).where(eq(schema.wallets.userId, userId)).limit(1),
      this.db
        .select()
        .from(schema.userVerifications)
  ...
  async updateUserRole(targetUserId: string, role: string, adminId: string) {
    ...
    if (!validRoles.includes(role)) throw new BadRequestException('INVALID_ROLE')
    ...
    await this.logAudit(adminId, 'user.role_change', 'user', targetUserId, { role: user.role }, { role })
  ...
  async suspendUser(targetUserId: string, adminId: string) {
    ...
    const [updated] = await this.db
      ...
      .where(and(eq(schema.users.id, targetUserId), eq(schema.users.status, 'active')))
      .returning({ id: schema.users.id, status: schema.users.status })

    await this.logAudit(adminId, 'user.suspend', 'user', targetUserId, { status: user.status }, { status: 'suspended' })
  ...
  async banUser(targetUserId: string, adminId: string) {
    ...
    if (!user) throw new NotFoundException('USER_NOT_FOUND')
    if (user.status === 'banned') throw new BadRequestException('USER_ALREADY_BANNED')
    ...
    await this.logAudit(adminId, 'user.ban', 'user', targetUserId, { status: user.status }, { status: 'banned' })

    return { id: targetUserId, status: 'banned' }
  }

  async adjustTokens(targetUserId: string, amount: number, reason: string, adminId: string) {
    ...
    await this.logAudit(adminId, 'user.token_adjust', 'user', targetUserId, null, { amount, reason })
  ...
  async updateConfig(key: string, value: unknown, changeReason: string, adminId: string) {
    const updated = await this.adminConfigService.update(key, value, adminId, changeReason)
    await this.logAudit(adminId, 'config.update', 'config', key, null, { value })
    return updated
  }
  ...
  async getTokenPacks() {
    const [packs, prices] = await Promise.all([
      this.db.select().from(schema.tokenPackDefinitions).orderBy(asc(schema.tokenPackDefinitions.sortOrder)),
  ...
  async updateTokenPack(
    ...
    await this.logAudit(adminId, 'token_pack.update', 'token_pack', packId, pack, data)
    return updated
  }
  ...
  async updateTokenPackPrice(priceId: string, price: string, adminId: string) {
    ...
    await this.logAudit(adminId, 'token_pack.price_update', 'token_pack_price', priceId, { price: priceRow.price }, { price })
    return updated
  }
  ...
  async listKycPending(type?: string) {
    ...
    if (type) {
      conditions.push(
        eq(schema.userVerifications.type, type as typeof schema.userVerifications.$inferSelect['type']),
      )
    }
  ...
  async approveKyc(verificationId: string, adminId: string) {
    ...
    if (verification.status !== 'pending') throw new BadRequestException('VERIFICATION_NOT_PENDING')
    ...
    await this.logAudit(adminId, 'kyc.approve', 'kyc', verificationId, { status: 'pending' }, { status: 'approved' })
  ...
  async rejectKyc(verificationId: string, reason: string, adminId: string) {
    ...
    await this.logAudit(adminId, 'kyc.reject', 'kyc', verificationId, { status: 'pending' }, { status: 'rejected', reason })
  ...
  async approveListing(listingId: string, adminId: string) {
    ...
    if (listing.moderationStatus !== 'pending') throw new BadRequestException('LISTING_NOT_PENDING')

    const publishedAt = new Date()
    const expiresAt = new Date(publishedAt.getTime() + (listing.durationDays ?? 30) * 86400_000)

    const [updated] = await this.db
      .update(schema.listings)
      .set({ moderationStatus: 'approved', status: 'active', publishedAt, expiresAt, updatedAt: new Date() })
      .where(eq(schema.listings.id, listingId))
      .returning()

    await this.logAudit(adminId, 'listing.approve', 'listing', listingId, { moderationStatus: 'pending' }, { moderationStatus: 'approved' })
  ...
  async rejectListing(listingId: string, reason: string, adminId: string) {
    ...
    const [updated] = await this.db
      .update(schema.listings)
      .set({ moderationStatus: 'rejected', status: 'draft', updatedAt: new Date() })
      .where(eq(schema.listings.id, listingId))
      .returning()

    await this.logAudit(adminId, 'listing.reject', 'listing', listingId, { moderationStatus: 'pending' }, { moderationStatus: 'rejected', reason })
  ...
  async getAuditLog(
    ...
    if (params.entityType) conditions.push(eq(schema.adminAuditLog.entityType, params.entityType))
    if (params.adminId) conditions.push(eq(schema.adminAuditLog.adminId, params.adminId))
    if (params.from) conditions.push(gte(schema.adminAuditLog.createdAt, new Date(params.from)))
    if (params.to) conditions.push(lt(schema.adminAuditLog.createdAt, new Date(params.to)))
    if (params.cursor) {
      const { createdAt, id } = decodeCursor(params.cursor)
      conditions.push(
        or(
          lt(schema.adminAuditLog.createdAt, createdAt),
          and(eq(schema.adminAuditLog.createdAt, createdAt), lt(schema.adminAuditLog.id, id)),
        ),
      )
    }
