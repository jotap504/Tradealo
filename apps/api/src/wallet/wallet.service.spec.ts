...
describe('WalletService', () => {
  ...
  describe('getBalance()', () => {
    ...
    it('throws NotFoundException when wallet not found', async () => {
      ...
      await expect(service.getBalance(USER_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('credit()', () => {
    ...
    it('throws BadRequestException when amount is zero or negative', async () => {
      await expect(
        service.credit(USER_ID, 0, 'registration_bonus'),
      ).rejects.toThrow(BadRequestException);
      await expect(
        service.credit(USER_ID, -1, 'registration_bonus'),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws NotFoundException when wallet update returns empty (user not found)', async () => {
      ...
      await expect(
        service.credit('nonexistent', 5, 'registration_bonus'),
  ...
  describe('debit()', () => {
    ...
    it('throws HttpException(402) when balance is insufficient', async () => {
      mockDb.transaction.mockImplementationOnce(
        async (fn: (tx: typeof mockDb) => Promise<unknown>) => {
          const tx = {
            select: jest.fn().mockReturnValue(qb([{ balance: 2 }])),
          };
          return fn(tx as any);
        },
      );
      await expect(
        service.debit(USER_ID, 10, 'listing_publish'),
      ).rejects.toThrow(
        new HttpException('INSUFFICIENT_BALANCE', HttpStatus.PAYMENT_REQUIRED),
      );
    });
    ...
    it('throws NotFoundException when wallet not found', async () => {
      ...
      await expect(
        service.debit('nonexistent', 3, 'listing_publish'),
      ).rejects.toThrow(NotFoundException);
    });

    it('throws BadRequestException when amount is zero or negative', async () => {
      await expect(
        service.debit(USER_ID, 0, 'listing_publish'),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('getFreeQuota()', () => {
    it('returns existing quota record', async () => {
      mockDb.select.mockReturnValueOnce(
        qb([{ quota: 5, used: 2, yearMonth: '2026-04', userId: USER_ID }]),
      );
  ...
  describe('consumeFreeQuota()', () => {
    ...
    it('throws UnprocessableEntityException when quota exhausted', async () => {
      ...
      await expect(service.consumeFreeQuota(USER_ID)).rejects.toThrow(
        UnprocessableEntityException,
      );
    });
  });
