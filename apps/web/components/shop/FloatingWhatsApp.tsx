...
export default function FloatingWhatsApp({ phoneNumber, shopName }: Props) {
  const prefersReduced = useReducedMotion();
  const clean = phoneNumber.replace(/\D/g, '');
  if (!clean) return null;

  const msg = shopName ? `Hola! Encontré tu tienda "${shopName}" en Trocalia` : undefined;
  const href = `https://wa.me/${clean}${msg ? `?text=${encodeURIComponent(msg)}` : ''}`;
