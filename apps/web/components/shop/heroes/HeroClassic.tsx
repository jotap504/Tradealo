...
export default function HeroClassic({ shop }: { shop: PublicShop }) {
  ...
  return (
    <div className="relative w-full overflow-hidden" style={{ minHeight: 380 }}>
      {shop.bannerUrl ? (
        <>
          <Image src={shop.bannerUrl} alt="Banner de tienda" fill priority className="object-cover" sizes="100vw" />
          <div
            className="absolute inset-0"
            style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.08) 0%, rgba(0,0,0,0.15) 30%, rgba(0,0,0,0.70) 70%, rgba(0,0,0,0.82) 100%)' }}
          />
        </>
      ) : (
        <motion.div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(135deg, var(--shop-primary) 0%, #818cf8 50%, var(--shop-primary) 100%)', backgroundSize: '300% 300%' }}
        ...
        <div className="max-w-6xl mx-auto">
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="flex flex-col gap-3">
            <motion.div variants={itemVariants} transition={{ duration: 0.5, ease: 'easeOut' }} className="flex items-end gap-4">
              {shop.logoUrl ? (
                <div className="relative shrink-0 rounded-2xl overflow-hidden" style={{ width: 96, height: 96, border: '4px solid var(--shop-surface)', boxShadow: '0 8px 32px rgba(0,0,0,0.30)' }}>
                  <Image src={shop.logoUrl} alt={displayName ?? 'Logo'} fill className="object-cover" />
                </div>
              ) : (
                <div className="shrink-0 rounded-2xl flex items-center justify-center text-3xl font-bold" style={{ width: 96, height: 96, backgroundColor: 'var(--shop-primary)', border: '4px solid var(--shop-surface)', boxShadow: '0 8px 32px rgba(0,0,0,0.30)', color: '#ffffff' }}>
                  {initial}
                </div>
              )}
              <div className="pb-1 min-w-0">
                <h1 className="text-3xl font-bold text-white leading-tight" style={{ textShadow: '0 2px 12px rgba(0,0,0,0.5)' }}>{displayName}</h1>
                {shop.tagline && <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.80)' }}>{shop.tagline}</p>}
              </div>
            </motion.div>
