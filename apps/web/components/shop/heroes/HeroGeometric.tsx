...
export default function HeroGeometric({ shop, config }: { shop: PublicShop; config: HeroConfig }) {
  ...
  const title2 = config.title2 ?? (shop.tagline ?? '');
  ...
  return (
    <div className="relative w-full min-h-[480px] md:min-h-[580px] flex flex-col overflow-hidden bg-[#030303]">
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-4 md:px-6 pt-16 pb-8">
      ...
      <div className="relative z-10 px-4 pb-8 md:pb-10">
        <div className="max-w-6xl mx-auto flex flex-col gap-3">
          <div className="flex items-end gap-4">
            {shop.logoUrl ? (
              <div className="relative shrink-0 rounded-2xl overflow-hidden" style={{ width: 80, height: 80, border: '3px solid rgba(255,255,255,0.20)', boxShadow: '0 8px 32px rgba(0,0,0,0.50)' }}>
                <Image src={shop.logoUrl} alt={displayName ?? 'Logo'} fill className="object-cover" />
              </div>
            ) : (
              <div className="shrink-0 rounded-2xl flex items-center justify-center text-2xl font-bold text-white" style={{ width: 80, height: 80, background: 'var(--shop-primary, #6366f1)', border: '3px solid rgba(255,255,255,0.20)', boxShadow: '0 8px 32px rgba(0,0,0,0.50)' }}>
                {initial}
              </div>
            )}
            <div className="pb-1 min-w-0">
              <p className="text-xl font-bold text-white leading-tight" style={{ textShadow: '0 2px 12px rgba(0,0,0,0.7)' }}>{displayName}</p>
